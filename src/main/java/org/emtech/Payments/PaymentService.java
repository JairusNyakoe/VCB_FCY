...
public class PaymentService {
    ...
    //  Submits forex payments to CBK and returns only filename and status.
    ...
    public Map<String, Object> submitPayments(LocalDate start, LocalDate end) {
        ...
        try (Connection conn = databaseConnection.dbConnection();
            ...
            log.error("Error fetching forex payments from DB: {}", e.getMessage(), e);
        }

        // Build CBK payload (Area 60)
        ...
        for (Props p : propsList) {
            ...
            dynamicItems.add(Map.of("Code", "1.2", "Value", p.getCustomerName())); // CUSTOMER NAME
        ...
        log.info("Submitting payments to CBK: {}", url);
        ...
        try {
            ...
            log.error("Error submitting payments to CBK: {}", e.getMessage(), e);
        }
