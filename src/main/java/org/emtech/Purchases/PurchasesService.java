...
public class PurchasesService {
    ...
    public Map<String, Object> submitPurchases(LocalDate start, LocalDate end) {
        ...
        try {
            ...
            return null; // app continues running
            ...
            log.error("Error submitting purchases to CBK", e);
            return null;
        }
    ...
    private void checkStatus(String fileName) {
        try {
            ...
            if (e.getCause() instanceof UnknownHostException) {
                log.error("No connection to CBK (DNS failure) while checking purchase status. Cannot resolve bankreturns.centralbank.go.ke");
            } else {
                log.error("No connection to CBK while checking purchase status for file {}. Network error: {}",
