    ...
    public void fetchAndSaveReceipts() {
        ...
        String selectQuery = """
         SELECT \s
            ...
            WHEN 'AGRICULTURE, HUNTING, FISHING AND FORESTRY'        THEN 'RG01'
                       ...
                       WHEN 'ELECTRICITY AND WATER'                              THEN 'RG13'
                       WHEN 'FINANCE AND INSURANCE'                              THEN 'RS03'
                       WHEN 'FOREIGN TRADE'                                      THEN 'RT0'
                       WHEN 'MANUFACTURING'                                      THEN 'RG07'
                       WHEN 'MINING AND QUARRYING'                               THEN 'RG06'
                       WHEN 'OTHER ENTERPRISES'                                  THEN 'RS08'
                       WHEN 'REAL ESTATE'                                        THEN 'RS08'
                       WHEN 'SOCIAL, COMMUNITY AND PERSONAL SERVICES'            THEN 'RS0'
        ...
        END AS SECTOR,
                REPLACE(cvm.LOCALE_VALUE,'&','AND')AS SECTORDESCRIPTION,
                  TO_CHAR(CGM.LCHG_TIME , 'DD-MM-YYYY HH24:MI:SS') TIMESTAMP,
        ...
        END AS SECTOR,
        REPLACE(cvm.LOCALE_VALUE,'&','AND')AS SECTORDESCRIPTION,
                   TO_CHAR(FAE.LCHG_TIME , 'DD-MM-YYYY HH24:MI:SS') TIMESTAMP,
                ...
                cvm.LOCALE_VALUE,
                        REPLACE(cvm.LOCALE_VALUE,'&','AND')AS SECTORDESCRIPTION,
                TO_CHAR( A.DEAL_DATE , 'DD-MM-YYYY HH24:MI:SS')DEAL_DATE,
         ...
         REPLACE(cvm.LOCALE_VALUE,'&','AND')AS SECTORDESCRIPTION,
                ...
                --gam.cif_id,
                cvm.LOCALE_VALUE,
                REPLACE(cvm.LOCALE_VALUE,'&','AND')AS SECTORDESCRIPTION, \s
                TO_CHAR( A.DEAL_DATE , 'DD-MM-YYYY HH24:MI:SS')DEAL_DATE,
                B.CCY_ONE_AMOUNT_CCY CRNCY,
                       ABS(B.CCY_ONE_AMOUNT_VALUE) AMT,
                --       ' ' PURPOSE,
